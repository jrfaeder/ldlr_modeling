...
A2 version:
- A changes: LDLR_init, k_recycle, k_degrade
- F changes: k_endo, k_off_surf
- Cluster 2 rescue uses softened abundance scaling

Run from ldlr_modeling/:
  python models/ldlr_model_a2.py p.Pro3Ile
"""
...
# =========================================================
# CLUSTER 2 RESCUE CONSTANT
# Current choice: 0.70
# Formula used only for cluster 2:
#   A_eff_model = CLUSTER2_A_FLOOR + (1 - CLUSTER2_A_FLOOR) * A_used
# =========================================================
CLUSTER2_A_FLOOR = 0.70
...
def choose_scores_by_cluster(cluster: int, F_raw: float, A_raw: float):
    ...
    if cluster == 2:
        ...
        note = (
            ...
            f"Abundance scaling will be softened with floor={CLUSTER2_A_FLOOR:.2f}."
        )
        return F_used, A_used, note, skip_sim, cluster
    ...
    if cluster == 3:
        ...
        note = (
            ...
            f"scales k_endo and k_off_surf; A_used=1.0."
        )
        return F_used, A_used, note, skip_sim, cluster
...
class LDLRModel:
    ...
    def get_effective_abundance(self) -> float:
        """
        Cluster-2 rescue:
        soften abundance penalty with a baseline floor.

        IMPORTANT constant to tune:
            CLUSTER2_A_FLOOR
        """
        A = max(self.abundance_score, EPS)

        if self.cluster == 2:
            A_rescued = CLUSTER2_A_FLOOR + (1.0 - CLUSTER2_A_FLOOR) * A
            return A_rescued
    ...
    def get_scaled_parameters(self):
        """
        A changes:
          - LDLR_init positively
          - k_recycle positively
          - k_degrade inversely

        F changes:
          - k_endo positively
          - k_off_surf inversely

        For cluster 2, A is softened using get_effective_abundance().
        """
        ...
        k_recycle_scaled = K_RECYCLE_BASE * A_eff_model
        k_degrade_scaled = K_DEGRADE_BASE / A_eff_model
        ...
        return {
            ...
            "k_recycle": k_recycle_scaled,
            "k_degrade": k_degrade_scaled,
...
begin parameters
    ...
    k_endo         {p["k_endo"]:.4f}
    k_recycle      {p["k_recycle"]:.4f}
    k_degrade      {p["k_degrade"]:.4f}
...
def main():
    ...
    print(f"k_recycle     = {params['k_recycle']:.4f}")
    print(f"k_degrade     = {params['k_degrade']:.4f}")
