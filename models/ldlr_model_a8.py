...
A8 version:
- keeps the A5 biology/mapping structure
- adds direct parameter overrides for search/sensitivity workflows

A targets:
- steady-state surface LDLR ratio vs WT steady-state surface reference

F targets:
- steady-state uptake ratio vs WT steady-state uptake reference

IMPORTANT MODEL FEATURES IN THIS VERSION
1. Receptor degradation reaction remains REMOVED:
   LDLR(...,loc~endosome) -> 0
   because it depleted the receptor pool and made surface LDLR / uptake collapse to zero.

2. The old shared trafficking parameter remains split:
   - k_recycle_ldlr : LDLR endosome -> surface
   - k_lyso_ldl     : LDL cargo endosome -> lysosome
   - k_degrade_ldl  : LDL degradation in lysosome

3. A scales receptor-side abundance parameters:
   - LDLR_init
   - k_recycle_ldlr

4. F scales uptake-side parameters:
   - k_endo
   - k_off_surf
...
6. NEW IN A8:
   ...
   cluster-based scaling. This is intended for sanity checks, random parameter
   variation, and Latin hypercube sampling workflows.

Run from ldlr_modeling/:
  python models/ldlr_model_a8.py p.Pro3Ile

Optional:
  python models/ldlr_model_a8.py p.Pro3Ile --t_end 200 --n_steps 1000

Optional direct parameter overrides:
  python models/ldlr_model_a8.py p.Pro3Ile --k_endo 1.2 --k_off_surf 0.8
"""
...
def choose_scores_by_cluster(cluster: int, F_raw: float, A_raw: float):
    ...
    # Cluster 2: abundance-defect, NO rescue
    if cluster == 2:
        F_used = 1.0
        A_used = A_eff
        note = (
            f"Cluster2 (abundance-defect, no rescue): F_used=1.0, "
            f"A_used={A_used:.3f} is applied directly to receptor-side abundance parameters."
        )
        return F_used, A_used, note, skip_sim, cluster
...
class LDLRModel:
    ...
    def get_effective_abundance(self) -> float:
        """
        No cluster-2 rescue in this version.
        Effective abundance is just A_used (floored by EPS).
        """
        return max(self.abundance_score, EPS)

    def get_scaled_parameters(self):
        """
        A -> receptor-side abundance / steady-state surface availability:
             - LDLR_init
             - k_recycle_ldlr
        ...
        LDL cargo parameters are kept constant for now:
             - k_lyso_ldl
             - k_degrade_ldl

        Optional param_overrides can directly replace any of these after
