...
- For cluster 2 only, weaken the penalty on recycling:
    k_recycle_ldlr = K_RECYCLE_LDLR_BASE * A^beta
  where beta < 1

Biological idea:
- A still keeps surface LDLR lower than WT
- recycling is penalized more gently than LDLR_init
- F still rescues uptake via k_endo and k_off_surf

Run from ldlr_modeling/:
  python models/ldlr_model_a7.py p.Leu8Gln
"""
...
def choose_scores_by_cluster(cluster: int, F_raw: float, A_raw: float):
    ...
    if cluster == 2:
        ...
        A_used = A_eff
        note = (
            f"Cluster2 (method 3C): A_used={A_used:.3f}, F_used={F_used:.3f}. "
            f"No abundance rescue; cluster-2 k_recycle_ldlr scales as A^{CLUSTER2_A_RECYCLE_BETA:.2f}."
        )
        return F_used, A_used, note, skip_sim, cluster
...
class LDLRModel:
    ...
    def get_effective_abundance(self) -> float:
        # No abundance rescue in method 3C
        return max(self.abundance_score, EPS)

    def get_cluster2_recycle_scale(self, A: float) -> float:
        return A ** CLUSTER2_A_RECYCLE_BETA
    ...
    def get_scaled_parameters(self):
        ...
        if self.cluster == 2:
            k_recycle_ldlr_scaled = K_RECYCLE_LDLR_BASE * self.get_cluster2_recycle_scale(A_eff_model)
        else:
...
def main():
    ...
    if cluster == 2:
        print(f"cluster2 recycle exponent beta = {CLUSTER2_A_RECYCLE_BETA:.2f}")
