...
A5_no_cluster2_rescue version:
- A targets steady-state surface LDLR ratio
- F targets steady-state uptake ratio

IMPORTANT MODEL CHANGES IN THIS VERSION
1. Receptor degradation reaction was REMOVED:
   LDLR(...,loc~endosome) -> 0
   because it depleted the receptor pool and made surface LDLR / uptake collapse to zero.

2. The old shared trafficking parameter was split:
   - k_recycle_ldlr : LDLR endosome -> surface
   - k_lyso_ldl     : LDL cargo endosome -> lysosome
   - k_degrade_ldl  : LDL degradation in lysosome

3. A now scales only receptor-side abundance parameters:
   - LDLR_init
   - k_recycle_ldlr
...
5. Cluster 2 rescue has been REMOVED entirely.
   Cluster 2 now uses raw A directly, just like its assigned A_used value.

Run from ldlr_modeling/:
  python models/ldlr_model_a5.py p.Pro3Ile

Optional:
  python models/ldlr_model_a5.py p.Pro3Ile --t_end 200 --n_steps 1000
"""
...
def main():
    ...
    print("=== Receptor/cargo-separated parameters ===")
