...
def choose_scores_by_cluster(cluster: int, F_raw: float, A_raw: float):
    ...
    # Cluster 2: abundance-defect rescue
    if cluster == 2:
        F_used = 1.0
        A_used = A_eff
        note = (
            f"Cluster2 (abundance-defect rescue): F_used=1.0, raw A_used={A_used:.3f}. "
            f"A will be softened with floor={CLUSTER2_A_FLOOR:.2f} for receptor-side abundance parameters."
        )
        return F_used, A_used, note, skip_sim, cluster
...
class LDLRModel:
    ...
    def get_effective_abundance(self) -> float:
        """
        Cluster-2 rescue is applied only to receptor-side abundance control.
        """
        A = max(self.abundance_score, EPS)

        if self.cluster == 2:
            return CLUSTER2_A_FLOOR + (1.0 - CLUSTER2_A_FLOOR) * A
